...
async def get_qualitative_analysis(company:str, period: str, query: str = ""):
    try:
        user_quer = f"Company: {company}, Period: {period}, Query: {query}"
        graph_executor = compile_graph()
        state = init_query_state(user_quer, "e9bd98df-2cf5-4d53-92ad-a14bbbcb5e9e")
        res = graph_executor.invoke(state)
        return res
    except Exception as e:
        print("RISK MATRIX:", repr(e))
        raise HTTPException(status_code=500, detail=str(e))
